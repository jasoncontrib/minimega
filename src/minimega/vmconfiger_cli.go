// Code generated by "vmconfiger -type BaseConfig,KVMConfig,ContainerConfig,AndroidConfig"; DO NOT EDIT
...
var vmconfigerCLIHandlers = []minicli.Handler{
	{
		HelpShort: "configures number-prefix",
		HelpLong: `Set the telephone network number prefix for newly launched VMs. For example:
...
`,
		Patterns: []string{
			"vm config number-prefix [value]",
		},
		Call: wrapSimpleCLI(func(c *minicli.Command, r *minicli.Response) error {
			if len(c.StringArgs) == 0 {
				r.Response = strconv.FormatInt(vmConfig.NumberPrefix, 10)
				return nil
			}
...
func (v *AndroidConfig) Info(field string) (string, error) {
	if field == "number-prefix" {
		return fmt.Sprintf("%v", v.NumberPrefix), nil
	}
...
func (v *AndroidConfig) Clear(mask string) {
	if mask == Wildcard || mask == "number-prefix" {
		v.NumberPrefix = 0
	}
}
